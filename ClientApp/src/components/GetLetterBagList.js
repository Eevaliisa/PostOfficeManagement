...
export class GetLetterBagList extends Component {
    ...
    async componentDidMount() {
        this.setState({ isFetching: true, shipmentId: this.props.id})
        const result = await fetch(`http://localhost:5000/api/letterbag/${this.props.id}/bagslist`);
    ...
    render() {
        if (this.state.letterBags.isFetching) {
            ...
            return(
            <div>
                <h1 className="list-title">Bags in shipment no {this.props.shipmentId}</h1>
